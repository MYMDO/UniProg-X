    ...
    return (
        <div className="bg-slate-900/80 border border-white/10 rounded-none p-6 backdrop-blur-md relative overflow-hidden">
            ...
            <h2 className="text-lg font-bold mb-4 text-white flex items-center gap-2 font-mono uppercase tracking-wider">
                <span className="text-blue-400">🔒</span> Option Bytes
            </h2>
            ...
            {optionBytes ? (
                <div className="space-y-4">
                    ...
                    <div className="bg-black/30 p-4 border border-white/5">
                        <div className="text-xs text-slate-400 mb-2 font-mono uppercase tracking-wider">Read Protection</div>
                        ...
                        {optionBytes.rdp === 0xCC && (
                            <div className="mt-3 p-2 bg-rose-900/20 border border-rose-500/30 text-rose-300 text-xs font-mono">
                    ...
                    {/* User Option Bytes */}
                    <div className="bg-black/30 p-4 border border-white/5">
                        <div className="text-xs text-slate-400 mb-3 font-mono uppercase tracking-wider">User Configuration</div>
                        <div className="space-y-2">
                            {getUserFlags().map((flag) => (
                                <div key={flag.name} className="flex items-center justify-between text-sm font-mono">
                                    <span className="text-slate-300">{flag.name}</span>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-slate-500">{flag.desc}</span>
                                        <span className={`px-2 py-1 text-xs ${flag.value ? 'bg-emerald-600/20 text-emerald-400' : 'bg-slate-700/20 text-slate-400'}`}>
                                            {flag.value ? 'ON' : 'OFF'}
                                        </span>
                                    </div>
                                </div>
                    ...
                    {/* Write Protection */}
                    <div className="bg-black/30 p-4 border border-white/5">
                        <div className="text-xs text-slate-400 mb-2 font-mono uppercase tracking-wider">Write Protection</div>
                        <div className="grid grid-cols-2 gap-2 font-mono text-xs">
                            <div>
                                <span className="text-slate-400">WRP0:</span>
                                <span className="text-cyan-400 ml-2">0x{optionBytes.wrp0.toString(16).padStart(2, '0').toUpperCase()}</span>
                            </div>
                            <div>
                                <span className="text-slate-400">WRP1:</span>
                                <span className="text-cyan-400 ml-2">0x{optionBytes.wrp1.toString(16).padStart(2, '0').toUpperCase()}</span>
                            </div>
                        </div>
            ...
            <div className="mt-4">
                <button
                    ...
                    className="w-full p-3 bg-cyan-600/20 hover:bg-cyan-600/30 text-cyan-400 border border-cyan-500/30 font-mono font-bold tracking-wider transition-all disabled:opacity-30"
                >
                    READ OPTION BYTES
                </button>
            </div>
            ...
            {/* Info */}
            <div className="mt-4 p-3 bg-blue-900/20 border border-blue-500/30 text-blue-300 text-xs font-mono">
